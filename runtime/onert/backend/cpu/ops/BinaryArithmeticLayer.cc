...
struct FloatOpFunctor
{
  ...
  void (*m_elementwiseFunc)(int, const nnfw::cker::BinaryArithmeticOpParamFloat &, const float *,
                            const float *, float *);
  void (*m_swappedArgsElementwiseFunc)(int, const nnfw::cker::BinaryArithmeticOpParamFloat &,
                                       const float *, const float *, float *);
  void (*m_lhsBroadcastFunc)(int, const nnfw::cker::BinaryArithmeticOpParamFloat &, const float,
                             const float *, float *);
  void (*m_rhsBroadcastFunc)(int, const nnfw::cker::BinaryArithmeticOpParamFloat &, const float,
                             const float *, float *);
  std::function<float(const float &, const float &)> m_genericFunc;

  template <typename OPERATOR, typename SWAPPEDARGSOPERATOR>
  FloatOpFunctor(const nnfw::cker::BinaryArithmeticOpParamFloat &op_params, const OPERATOR &,
                 const SWAPPEDARGSOPERATOR &)
      : m_op_params(op_params)
  {
    auto op_func1 =
        nnfw::cker::optimized::getBinaryOpWithActivationImplFloat<OPERATOR>(m_op_params);
    auto op_func2 =
        nnfw::cker::optimized::getBinaryOpWithActivationImplFloat<SWAPPEDARGSOPERATOR>(m_op_params);
    ...
    m_genericFunc = [](const float &a, const float &b) -> float {
  ...
  void operator()(const IPortableTensor *lhs, const IPortableTensor *rhs, IPortableTensor *output)
  {
    ...
    const bool need_broadcast =
        nnfw::cker::ProcessBroadcastShapes(lhsShape, rhsShape, &m_op_params);
    ...
    if (need_broadcast)
    {
      if (m_op_params.broadcast_category ==
          nnfw::cker::BroadcastableOpCategory::kFirstInputBroadcastsFast)
      {
        nnfw::cker::optimized::BinaryBroadcastFiveFold(
            m_op_params, false, lhsShape, lhsBuffer, rhsShape, rhsBuffer, outputShape, outputBuffer,
            m_elementwiseFunc, m_lhsBroadcastFunc);
      }
      else if (m_op_params.broadcast_category ==
               nnfw::cker::BroadcastableOpCategory::kSecondInputBroadcastsFast)
      {
        nnfw::cker::optimized::BinaryBroadcastFiveFold(
            m_op_params, true, lhsShape, lhsBuffer, rhsShape, rhsBuffer, outputShape, outputBuffer,
            m_swappedArgsElementwiseFunc, m_rhsBroadcastFunc);
      }
      else
      {
        nnfw::cker::reference::BroadcastBinaryArithmeticOpSlow(m_op_params, lhsShape, lhsBuffer,
                                                               rhsShape, rhsBuffer, outputShape,
                                                               outputBuffer, m_genericFunc);
      }
    }
    else
    {
                                      ...
                                      const ArithmeticType arithmetic_type)
{
  ...
  switch (_lhs->data_type())
  {
      ...
      switch (arithmetic_type)
      {
        case ArithmeticType::kAdd:
          ...
          _kernel = std::bind(&eval<nnfw::cker::BinaryArithmeticOpType::ADD, uint8_t,
                                    nnfw::cker::BinaryArithmeticOpParamQuantized>,
    ...
    case OperandType::FLOAT32:
    {
      ...
      switch (arithmetic_type)
      {
        case ArithmeticType::kAdd:
          ...
          break;
        case ArithmeticType::kSub:
          _kernel = FloatOpFunctor(op_params, BinaryOpFuncSubFloat(),
                                   BinaryOpFuncSwapArgs<BinaryOpFuncSubFloat>());
          break;
        case ArithmeticType::kMul:
        ...
        case ArithmeticType::kDiv:
#ifdef __aarch64
          _kernel = FloatOpFunctor(op_params, BinaryOpFuncDivFloat(),
                                   BinaryOpFuncSwapArgs<BinaryOpFuncDivFloat>());
#else
          _kernel = std::bind(eval<nnfw::cker::BinaryArithmeticOpType::DIV, float,
                                   nnfw::cker::BinaryArithmeticOpParamFloat>,
                              std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                              op_params);
#endif
          break;
    ...
    case OperandType::INT32:
    {
      ...
      {
        case ArithmeticType::kAdd:
          _kernel = std::bind(eval<nnfw::cker::BinaryArithmeticOpType::ADD, int32_t,
                                   nnfw::cker::BinaryArithmeticOpParamQuantized>,
                              std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                              op_params);
          break;
        case ArithmeticType::kSub:
          _kernel = std::bind(eval<nnfw::cker::BinaryArithmeticOpType::SUB, int32_t,
                                   nnfw::cker::BinaryArithmeticOpParamQuantized>,
                              std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                              op_params);
          break;
        case ArithmeticType::kMul:
          _kernel = std::bind(eval<nnfw::cker::BinaryArithmeticOpType::MUL, int32_t,
                                   nnfw::cker::BinaryArithmeticOpParamQuantized>,
                              std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                              op_params);
          break;
        case ArithmeticType::kDiv:
          _kernel = std::bind(eval<nnfw::cker::BinaryArithmeticOpType::DIV, int32_t,
                                   nnfw::cker::BinaryArithmeticOpParamQuantized>,
                              std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                              op_params);
          break;
